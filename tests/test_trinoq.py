...
class TestCLIOptions:
    ...
    @patch("trinoq.get_query")
    def test_noti_flag(
        self, mock_get_query, mock_get_args, mock_execute, mock_send_notification
    ):
...
class TestCompactFlag:
    ...
    @patch("trinoq.get_args")
    @patch("trinoq.get_query")
    def test_compact_prints_data_only(
        self, mock_get_query, mock_get_args, mock_execute
    ):
        """Test that --compact prints only the data without labels."""
        from trinoq import app
        import pandas as pd
        ...
        with patch("sys.stdout", new_callable=StringIO) as mock_stdout:
            app()
            output = mock_stdout.getvalue()
            # Should contain data but not labels
            assert "col" in output
            assert "In[query]:" not in output
            assert "Out[df]:" not in output
